...
def test_report_payload_includes_selected_level_percentiles(tmp_path: Path) -> None:
    source = tmp_path / "hts_right.npy"
    source.write_bytes(b"D1 raw bytes")
    anchors = _anchors()
    anchors.group_metadata["thumb:lateral"] = {
        "distribution_parameter": "alpha",
        "distribution_values": anchors.observed_parameters[:5].tolist(),
    }
    payload = build_report_payload(
        anchors, input_path=source, hand_side="right", config={}
    )
    group = payload["groups"][0]
    assert group["distribution_parameter"] == "alpha"
