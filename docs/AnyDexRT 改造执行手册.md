...
- [x]  `geort/contact/auto_label_contacts.py`：从已有 D1 raw `[T,21,3]` 自动计算拇指×食/中/无名/小指四个指尖距离序列；检测每对的低距离物理下限平台，平台帧标正样本、明显更大距离标负样本、阈带内的连续模糊段整体丢弃。输出 `contact_labels_<side>.npz` 与距离直方图、正/负/丢弃计数、阈带位置的 JSON/HTML 报告。阈值与平台检测参数可配；不录制、也不反向构造悬停难负样本。
- [x]  `geort/contact/contact_model.py`：逐指对 6D（拇指/目标指指尖坐标）MLP 二分类器，BCE、独立优化器；仅使用自动标注的清晰样本，按连续时间尾段 held-out，训练统计量只拟合训练段。F1 仅留档，已知没有悬停难负样本会偏乐观。代码与模块级测试已完成，真实 D1 分类器尚未训练。
- [ ]  推理端集成：`load_model` 可选加载接触模型并输出逐指对概率；触发时在映射位置附近，用精确 FK 在关节限位内搜索拇指–目标指距离最小的可行捏合姿态。未提供接触 checkpoint 时保持当前 realtime 行为不变。

**人工（H4，已确认）**：右手 D1 自动标注报告中的四个指对距离直方图、平台阈带和正/负/丢弃数量已目检通过；不再人工录制。

**验收**：每指对正/负样本各不少于 500 帧且平台期清晰；held-out 为完整连续时间段；分类 F1 留档但不作为硬门槛；实测小物体捏取触发时机不劣于旧 15mm 阈值。
