...
## 2.6 R/L 手性 token 补查（M1 修令）

检索补充口径：`-R-`、`-L-`、`URDF_R`、`URDF_L`、`_R.`、`_L.`；已在第 2 节以 `right/custom_right` 统计的纯路径命中不重复计数。下表的“本轮 M1”只表示 manifest → parity → analytic FK 的实际执行链；未标记的项保留给所属工位里程碑。

### 产线代码／输入资产

| 文件:行号 | 命名约定原文（节录） | 本轮 M1 |
|---|---|---|
| `configs/hands/custom_right.yaml:2-5` | `urdf: assets/custom_right/URDF_R.urdf`; `hand_config: custom_right`; `side: R` | 是：manifest 显式 token 来源 |
| `configs/hands/custom_left.yaml:2-5` | `urdf: assets/custom_left/URDF_L.urdf`; `hand_config: custom_left`; `side: L` | 是：manifest 显式 token 来源 |
| `geort/analytic_fk.py:16-41,59-85` | `F{finger}-{side}-{role}`；`tip_link_names(side)`；`side: str = "R"` | 是：M1 解析 FK token 模板；不做 URDF 侧别猜测 |
| `geort/anchor/compat.py:19-38` | `make_analytic_tip_callback(..., side=...)`; `AnalyticFK(..., side=side)` | 是：M1 token 传递边界 |
| `geort/anchor/parity.py:88-105,145-157` | `side: str = "R"`; `make_analytic_tip_callback(... side=side)`；`side=manifest.side` | 是：manifest parity 消费 token |
| `geort/config/custom_right.json:3-201` | `URDF_R.urdf`；`F{1..5}-R-{MCP2,MCP1,PIP,DIP}` keypoint/joint order | 是：M1 右手输入契约 |
| `geort/config/custom_left.json:3-201` | `URDF_L.urdf`；`F{1..5}-L-{MCP2,MCP1,PIP,DIP}` keypoint/joint order | 是：M1 左手输入契约 |
| `assets/custom_right/URDF_R.urdf:57-684` | `<link/joint name="F{1..5}-R-{MCP2,MCP1,PIP,DIP}">` | 是：M1 右手 URDF 命名来源 |
| `assets/custom_left/URDF_L.urdf:57-724` | `<link/joint name="F{1..5}-L-{MCP2,MCP1,PIP,DIP}">` | 是：M1 左手 URDF 命名来源 |
| `geort/trainer.py:457` | `tip_link_names = ["F1-R-DIP", ..., "F5-R-DIP"]` | 否：训练／nullspace 工位 |
| `geort/mocap/calibrate_custom_aa_limits.py:158` | `joint_name = f"{prefix}-R-MCP2"` | 否：AA 标定工位 |
| `geort/mocap/search_custom_aa_limits.py:30-52` | `RIGHT_MANUAL_CLOSURE_PRIOR_V1`; `LEFT_MANUAL_CLOSURE_PRIOR_V1`; 从 hand 名推 side | 否：AA 搜索工位；保留其既有推断行为，未纳入 M1 |

### 评测代码

| 文件:行号 | 命名约定原文（节录） | 本轮 M1 |
|---|---|---|
| `scripts/evaluate_c2el_s42.py:46,67-68` | `custom_right_with_rot.npz`; `get_config('custom_right')` | 否：C2eL 专项评测 |
| `scripts/evaluate_anchor_pc1beta1_fix.py:123-138` | `anchors_*right*`; `get_config("custom_right")` | 否：锚点方向修复评测 |
| `outputs/final_matrix/evaluate_final_matrix.py:18,53` | right target/bundle/checkpoint manifest | 否：终验矩阵归档脚本 |

### Realtime

| 文件:行号 | 命名约定原文（节录） | 本轮 M1 |
|---|---|---|
| `geort/mocap/hts_realtime_inference.py:631-644` | `infer_hand_side(hand, hand_side)`；`--hand-side ... left, right` | 否：realtime 工位；已有 CLI side 解析 |
| `geort/mocap/hts_realtime_inference.py:22,47,649` | `hts_right_mocap`; `DEFAULT_C2B_S42_CHECKPOINT`; `--hand custom_right` | 否：realtime 默认右手路径 |
| `geort/contact/runtime.py:142-153` | `Current custom_right ...`; custom-right TIP order | 否：可选接触工位 |

### 测试

| 文件:行号 | 命名约定原文（节录） | 本轮 M1 |
|---|---|---|
| `tests/test_pipeline_manifest.py:19,37-49` | `URDF_L.urdf`; `AnalyticFK(..., side="L")` | 是：M1 显式 left token 回归 |
| `tests/test_custom_right_urdf_limits.py:22-44` | `URDF_R.urdf` / `URDF_L.urdf`; `F{2..5}-{R/L}-MCP2` | 否：URDF 限位验收 |
| `tests/test_search_custom_aa_limits.py:26-68,270-301` | `F{2..5}-{R/L}-MCP2` reference/prior | 否：AA 搜索 fixture |
| `tests/test_build_target_cloud.py:41-48,127,178-183,262` | `F*-R-*` joint fixtures | 否：点云测试 fixture |
| `tests/test_analytic_fk.py:45,123`; `test_analytic_fk_rotations.py:19`; `test_trainer_device_contract.py:19` | `assets/custom_right/URDF_R.urdf` | 否：既有右手回归 fixture；M1 新增 left test 单列如上 |

### 死码／历史归档

| 文件:行号 | 命名约定原文（节录） | 本轮 M1 |
|---|---|---|
| `scripts/archive/tmp_evaluations/evaluate_c2be_motion.py:33-42` | `get_config('custom_right')`; `custom_right_with_rot.npz` | 否：`scripts/archive` 历史评测 |
| `outputs/final_matrix/build_custom_right_with_rot.py:44` | `AnalyticFK(config["urdf_path"], ...)` | 否：已归档 right rotation 变体生成器 |
| `outputs/final_matrix/supplement/run_supplement.py:12,40` | `get_config('custom_right')`; `hts_right.npy` | 否：已归档终验 supplement |
