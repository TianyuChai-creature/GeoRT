...
def cloud_shape(hn,rn):
 ...
 return rows
def main():
 p=argparse.ArgumentParser(); p.add_argument('--checkpoint',type=Path,required=True);p.add_argument('--output-json',type=Path,required=True);p.add_argument('--output-md',type=Path,required=True);a=p.parse_args()
