...
def _selected_percentiles(selected: np.ndarray, distribution: object) -> list[float]:
    values = np.sort(np.asarray(distribution, dtype=np.float64).reshape(-1))
    values = values[np.isfinite(values)]
    if values.size == 0:
        raise ValueError("anchor distribution contains no finite values")
    if values.size == 1:
        return [100.0] * len(selected)
    ranks = np.searchsorted(values, selected, side="right") - 1
    ranks = np.clip(ranks, 0, values.size - 1)
    return (100.0 * ranks / (values.size - 1)).astype(float).tolist()
...
def build_report_payload(
    ...
    *,
    ...
    for start in range(0, 50, 5):
        ...
        observed = anchors.observed_parameters[rows]
        groups.append(
            {
                "finger": finger,
                ...
                "diagnostics": anchors.group_metadata.get(key, {}),
                "distribution_parameter": diagnostics.get(
                    "distribution_parameter", "selected_parameter"
                ),
                "selected_percentiles": _selected_percentiles(
                    observed, diagnostics.get("distribution_values", observed)
                ),
            }
        )
    return {
